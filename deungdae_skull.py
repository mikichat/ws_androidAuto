...
class DeungdaeSkull:
    """
    등대와 해골 관련 자동화를 처리하는 클래스
    """
    def __init__(self):
        self.buttons = {
            'deungdae': 'images/deungdae_skull/deungdae.png',
            'skull': 'images/deungdae_skull/skull.png',
            'bogi': 'images/deungdae_skull/bogi.png',
            'gonggyeok': 'images/deungdae_skull/gonggyeok.png',
            'gyunbaechi': 'images/deungdae_skull/gyunbaechi.png',
            'chuljeong': 'images/deungdae_skull/chuljeong.png'
        }
    ...
    def find_image_on_screen(self, template_path, threshold=0.8):
        ...
        template = cv2.imread(template_path)  # 컬러로 템플릿 이미지 로드
        if template is None:
        ...
        try:
            result = cv2.matchTemplate(screenshot, template, cv2.TM_CCOEFF_NORMED)  # 컬러 이미지로 매칭
    ...
    def click_if_image_found(self, button_name):
        ...
        # 해골 버튼은 임계값을 0.5로 낮추고, 나머지는 0.8 유지
        threshold = 0.8 if button_name == 'skull' else 0.8
    ...
    def click_all_buttons(self):
        ...
        if self.click_if_image_found('deungdae'):
            ...
            if self.click_if_image_found('skull'):
                ...
                os.system('adb shell input tap 64 143')  # 나가기 버튼 클릭
                return False
            else:
                print("해골을 찾을 수 없습니다.")
                return False
    ...
    def run(self):
        ...
        print("등대, 해골, 보기, 공격, 균등배치, 출정 버튼을 순서대로 클릭하려면 't'를 누르세요.")
