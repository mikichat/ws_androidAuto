...
class DeungdaeSkull(DeungdaeOperation):
    """
    등대의 해골 작업을 처리하는 클래스
    """
    ...
    def execute(self):
        ...
        # 해골 버튼 클릭
        if not self.click_if_image_found('skull'):
            self.logger.warning("해골 버튼을 찾을 수 없습니다.")
            # 나가기 버튼을 클릭하고 종료
        ...
        # 공격 버튼 클릭
        if not self.click_if_image_found('gongyeok'):
            self.logger.warning("공격 버튼을 찾을 수 없습니다.")
            # 나가기 버튼을 클릭하고 종료
            self.click_if_image_found('nagagi')
            return False
            
        # 균등배치 버튼 클릭
        if not self.click_if_image_found('gyunbaechi'):
            self.logger.warning("균등배치 버튼을 찾을 수 없습니다.")
            # 나가기 버튼을 클릭하고 종료
            self.click_if_image_found('nagagi')
            return False
            
        # 출정 버튼 클릭
        if not self.click_if_image_found('chuljeong'):
            self.logger.warning("출정 버튼을 찾을 수 없습니다.")
            # 나가기 버튼을 클릭하고 종료
            self.click_if_image_found('nagagi')
            return False
